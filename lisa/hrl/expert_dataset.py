...
class ExpertDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.metaworld = False

        if 'aug' in self.kwargs and self.kwargs['aug']:
            if 'metaworld' not in expert_location:
                ## From the LORL paper
        ...
        elif 'metaworld' in expert_location:
            self.normalize_state_dim = np.array((3,224,224))
            self.metaworld = True
        ...
        if self.length < min_size:
            self.dataset_length = min_size
        else:
            self.dataset_length = self.length
    ...
    def __getitem__(self, i):
        ...
        if not self.full_traj:
            ...
            if self.metaworld:
                state = self.trajectories["features"][traj_idx][i]
                next_state = self.trajectories["features"][traj_idx][i + 1]
            else:
            ...
            if self.metaworld:
                states = self.trajectories["features"][i]
...
def load_trajectories(expert_location: str,
    ...
    if os.path.isfile(expert_location) or os.path.isdir(expert_location):
        ...
        if 'babyai' in expert_location:
            ...
            trajs = load_metaworld_data(expert_location, **kwargs)
        elif 'calvin' in expert_location:
        ...
        perm = np.arange(len(trajs["language"]))
...
def load_lorel_data(expert_location, **kwargs):
    ...
    if 'sawyer' in expert_location or 'metaworld' in expert_location:
...
def load_metaworld_data(expert_location, **kwargs):
    trajs = {"features": [], "actions": [], "rewards": [], "lengths": [], "language": [], "dones": []}
    data = pickle.load(open(expert_location, 'rb'))
    num_trajs, traj_len = data['actions'].shape[0], data['actions'].shape[1]
    # if "state" in data.keys() and kwargs['use_state']:
    #     trajs["states"] = data["state"]
    ...
    # trajs["all_features"] = trajs["features"]
    # trajs["all_actions"] = trajs["actions"]

    # idx = np.arange(0, 140, 7)  # TODO
    # trajs["features"] = trajs["features"][:,idx,:]
    # trajs["actions"] = trajs["features"][:,idx,:]
    # trajs["rewards"] = trajs["rewards"][:,idx,:]
    # trajs["lengths"] = trajs["lengths"][:,idx,:]
    # trajs["dones"] = trajs["dones"][:,idx,:]
