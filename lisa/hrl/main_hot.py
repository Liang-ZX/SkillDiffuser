...
def train(args):
    ...
    if args.wandb:
        ...
        wandb.init(
            name=exp_name,
            group=args.method,
            project=f'skill_metaworld_ML_pluo_{args.env.name}',  # TODO frozen  diff_hot_A100_LN
            config=dict(args),
            # entity='language-rl',
            # mode="offline",
        )
    ...
    if args.resume:
        ...
        # TODO
        checkpoint['model'].pop('option_selector.option_dt.embed_timestep.weight')
        checkpoint['model'].pop('diffuser.embed_ln.weight')
        checkpoint['model'].pop('diffuser.embed_ln.bias')

        for key in list(checkpoint['model'].keys()):
            if key.startswith('diff_trainer.model.inv_model') or key.startswith('diff_trainer.ema_model.inv_model'):
                    checkpoint['model'].pop(key)

        # for key in list(checkpoint['model'].keys()):
        #     if key.startswith('option_selector.option_dt.embed_state'):
        #         checkpoint['model'].pop(key)

        # checkpoint['model']['diffuser.embed_ln.weight'] = checkpoint['model']['diffuser.embed_ln.weight'][5:]
        # checkpoint['model']['diffuser.embed_ln.bias'] = checkpoint['model']['diffuser.embed_ln.bias'][5:]
        ...
        if ".pt" in args.diffuser.loadpath:
            model.diff_trainer.load_diff_only(args.diffuser.loadpath)
