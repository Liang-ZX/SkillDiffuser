...
class R3MEncoder(nn.Module):
    ...
    def __init__(self, hidden_size, ch=3):
        super().__init__()
        self.alignDim = nn.Linear(2048, hidden_size)

    def forward(self, observations):
        states = observations
        hidden = self.alignDim(states)

        return hidden


class R3MEncoder2(nn.Module):
    ...
    def forward(self, observations):
        states = observations
        hidden = self.act_fn(self.fc1(states))
