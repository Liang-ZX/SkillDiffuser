...
class DecEncoder(TrajectoryModel):
    ...
    def __init__(
        ...
        self.embed_ln = nn.LayerNorm(hidden_size)  # TODO
    ...
    def encode(self, states, actions, timesteps, options=None, word_embeddings=None, attention_mask=None):
        ...
        if self.use_options:
            ...
            stacked_inputs = torch.cat((actions, state_embeddings, option_embeddings), dim=2)   # (B, seq_length, )
    ...
    def decode(self, traj_out, states, actions, timesteps, options=None, word_embeddings=None, attention_mask=None):
        if self.use_options:
            # get predictions
            # predict next state given option, state and action. skip the last state for prediction
            if isinstance(self.state_dim, int):
                state_preds = self.predict_state(traj_out[:, :, self.act_dim:self.act_dim+self.state_dim])[:, :-1, :]
            else:
                state_preds = None
            ...
            if self.predict_q:
                ...
                q_preds = self.predict_q(traj_out[:, :, self.act_dim:self.act_dim+self.state_dim])
